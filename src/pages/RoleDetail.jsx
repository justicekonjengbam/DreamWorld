...
import { getRoleById, getCharactersByRole } from '../data/characters'
...
function RoleDetail() {
  ...
  const members = getCharactersByRole(id)
    ...
    <div className="role-detail page">
      <div className="container">
        ...
        <div className="role-hero">
          <div className="role-icon-large" style={{ background: role.color }}></div>
          ...
          <div className="members-grid">
            {members.map((member) => (
              <Link to={`/characters/${member.id}`} key={member.id} className="member-link">
                <Card>
                  <img src={member.avatar} alt={member.name} className="member-photo" />
                  <h3>{member.name}</h3>
